...
internal class LauncherLogic : IDisposable
{
    ...
    public static LauncherConfig Config => LauncherConfig.Instance;
    ...
    {
        if ((string.IsNullOrWhiteSpace(Config.GamePath) && Config.GamePath == path) || !Directory.Exists(path))
        {
            return null;
        }
        ...
        lastFindSubnauticaTask = Task.Factory.StartNew(() =>
        {
            ...
            nitroxEntryPatch = new NitroxEntryPatch(() => Config.GamePath);
    ...
    internal async Task StartMultiplayerAsync()
    {
        ...
        try
        {
            File.Copy(
                ...
                Path.Combine(Config.GamePath, "Subnautica_Data", "Managed", initDllName),
                true
            );
        }
        ...
        if (QModHelper.IsQModInstalled(Config.GamePath))
        {
    ...
    {
        string subnauticaPath = Config.GamePath;
        string subnauticaLaunchArguments = Config.LaunchArguments;
