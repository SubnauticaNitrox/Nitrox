...
internal static class Downloader
{
        ...
        try
        {
            ...
            using (StreamReader sr = new(response.GetResponseStream()))
            {
                ...
                for (int i = 0; i < data.Count; i++)
                {
                    ...
                    blogs.Add(new NitroxBlog(title, dateTime, new Uri(url), image));
                }
            }
        }
        catch (Exception ex)
    ...
    public async static Task<Version> GetNitroxLatestVersionAsync(CancellationToken cancellationToken = default)
    {
        try
        {
            using WebResponse response = await GetResponseFromCacheAsync(LATEST_VERSION_URL);
    ...
    private static async Task<WebResponse> GetResponseFromCacheAsync(string url, CancellationToken cancellationToken = default)
    {
