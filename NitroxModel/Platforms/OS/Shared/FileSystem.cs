...
namespace NitroxModel.Platforms.OS.Shared
{
    public abstract class FileSystem
    {
        ...
        public virtual Process OpenOrExecuteFile(string file)
        {
            ...
            {
                DisplayStatusCode(StatusCoe.FILE_SYSTEM_ERR, true, "Missing file path: " + nameof(file));
            }
        ...
        public string MakeRelativePath(string fromPath, string toPath)
        {
            ...
            {
                DisplayStatusCode(StatusCoe.FILE_SYSTEM_ERR, true, "A path to a file was missing" + nameof(fromPath));
            }
            if (string.IsNullOrEmpty(toPath))
            {
                DisplayStatusCode(StatusCoe.FILE_SYSTEM_ERR, true, "A path to a file was missing" + nameof(toPath));
            }
        ...
        public string ZipFilesInDirectory(string dir, string outputPath = null, string fileSearchPattern = "*", bool replaceFile = false)
        {
            ...
            {
                DisplayStatusCode(StatusCoe.FILE_SYSTEM_ERR, true, "Invalid directory path" + nameof(dir));
            }
            dir = Path.GetFullPath(dir);
            if (!Directory.Exists(dir))
            {
                DisplayStatusCode(StatusCoe.FILE_SYSTEM_ERR, true, "Invalid directory" + nameof(dir));
            }
            ...
            {
                DisplayStatusCode(StatusCoe.FILE_SYSTEM_ERR, true, $"The file '{outZipFullName}' already exists");
            }
                ...
                switch ((uint)ex.HResult)
                {
                    case 0x80070498:
                        ...
                        try
                        {
                            ...
                            {
                                // ignored
                                DisplayStatusCode(StatusCoe.FILE_SYSTEM_ERR, true, "An error occured while modifying files");
                            }
                        }
                        catch (Exception ex2)
                        {
                            DisplayStatusCode(StatusCoe.FILE_SYSTEM_ERR, true, ex2 + $"Failed to replace file '{source}' with '{target}' which is on another drive");
                            return false;
                        }
                        break;
                    default:
                        // No special handling implemented for error, abort.
                        DisplayStatusCode(StatusCoe.FILE_SYSTEM_ERR, true, $"Unhandled file replace of '{source}' with '{target}' with HRESULT: 0x{ex.HResult:X}");
                        return false;
                }
            }
            return true;
