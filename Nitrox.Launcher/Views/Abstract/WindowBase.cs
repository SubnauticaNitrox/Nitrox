...
public class WindowBase<TViewModal> : ReactiveWindow<TViewModal> where TViewModal : class
{
    ...
    /// </summary>
    protected void RegisterModal<TModal, TModalViewModel>(Func<Interaction<TModalViewModel, TModalViewModel?>> interactionGetter) where TModal : ModalBase<TModalViewModel>, new() where TModalViewModel : class, IModalViewModel
    {
        this.WhenActivated(d => d(interactionGetter().RegisterHandler(DefaultShowDialogAsync<TModal, TModalViewModel>)));
    }

    private async Task DefaultShowDialogAsync<TModal, TModalViewModel>(InteractionContext<TModalViewModel, TModalViewModel?> interaction) where TModal : ModalBase<TModalViewModel>, new() where TModalViewModel : class, IModalViewModel
    {
