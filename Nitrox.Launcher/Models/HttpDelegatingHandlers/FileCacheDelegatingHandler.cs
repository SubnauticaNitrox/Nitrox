...
internal sealed class FileCacheDelegatingHandler : DelegatingHandler
{
    ...
    private static string GetCacheFilePathForRequest(HttpRequestMessage request)
    {
        ...
        return Path.Combine(Path.GetTempPath(), $"nitrox_{string.Join('_', $"{uri.Host}{uri.LocalPath}".ReplaceInvalidFileNameCharacters('_').Split('_').Select(s => s[0]))}_{Convert.ToHexStringLower(uri.ToString().AsMd5Hash())}.cache");
    }
}
