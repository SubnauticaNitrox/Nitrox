...
namespace NitroxClient.GameLogic
{
    public class PDAManagerEntry
    {
        ...
        {
            packetSender.SendIfGameCode(new PDAEntryAdd(entry.techType.ToDto(), entry.progress, entry.unlocked));
        }

        public void Progress(PDAScanner.Entry entry, NitroxId nitroxId)
        {
            packetSender.SendIfGameCode(new PDAEntryProgress(entry.techType.ToDto(), entry.progress, entry.unlocked, nitroxId));
        }

        public void Remove(PDAScanner.Entry entry)
        {
            packetSender.SendIfGameCode(new PDAEntryRemove(entry.techType.ToDto()));
        }

        public void LogAdd(PDALog.Entry entry)
        {
            packetSender.SendIfGameCode(new PDALogEntryAdd(entry.data.key, entry.timestamp));
        }
    }
}
