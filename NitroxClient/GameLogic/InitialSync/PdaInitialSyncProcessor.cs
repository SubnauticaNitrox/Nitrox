...
namespace NitroxClient.GameLogic.InitialSync
{
    public class PdaInitialSyncProcessor : InitialSyncProcessor
    {
        ...
        {
            Log2.Instance.Log(NLogType.Info, "Received initial sync packet with " + entries.Count + " encyclopedia entries");
        ...
        private void SetPDAEntryComplete(List<TechTypeModel> pdaEntryComplete)
        {
            ...
            Log2.Instance.Log(NLogType.Info, "PDAEntryComplete Save:" + pdaEntryComplete.Count + " Read Partial Client Final Count:" + complete.Count);
        ...
        private void SetPDAEntryPartial(List<PDAEntry> entries)
        {
            ...
            Log2.Instance.Log(NLogType.Info, "PDAEntryPartial Save :" + entries.Count + " Read Partial Client Final Count:" + partial.Count);
        }
        
        private void SetKnownTech(List<TechTypeModel> techTypes)
        {
            Log2.Instance.Log(NLogType.Info, "Received initial sync packet with " + techTypes.Count + " known tech types");
        ...
        {
            Log2.Instance.Log(NLogType.Info, "Received initial sync packet with " + logEntries.Count + " pda log entries");
