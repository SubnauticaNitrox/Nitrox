...
public class FlareMetadataProcessor : EntityMetadataProcessor<FlareMetadata>
{
    public override void ProcessMetadata(GameObject gameObject, FlareMetadata metadata)
    {
        ...
        {
            DisplayStatusCode(StatusCode.subnauticaError, false, $"[{nameof(FlareMetadataProcessor)}] Can't apply metadata to {gameObject} because it doesn't have a {nameof(Flare)} component");
            return;
        }
