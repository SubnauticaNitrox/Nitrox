...
public class ExosuitMetadataProcessor : VehicleMetadataProcessor<ExosuitMetadata>
{
    ...
    public override void ProcessMetadata(GameObject gameObject, ExosuitMetadata metadata)
    {
        ...
        {
            DisplayStatusCode(StatusCode.SUBNAUTICA_ERROR, $"[{nameof(ExosuitMetadataProcessor)}] Could not find {nameof(Exosuit)} on {gameObject}");
            return;
        }
        if (!gameObject.TryGetComponent(out SubName subName))
        {
            DisplayStatusCode(StatusCode.SUBNAUTICA_ERROR, $"[{nameof(ExosuitMetadataProcessor)}] Could not find {nameof(SubName)} on {gameObject}");
            return;
        }
