...
namespace NitroxClient.GameLogic
{
    public class ItemContainers
    {
        ...
        public void BroadcastItemAdd(Pickupable pickupable, Transform containerTransform)
        {
            ...
            {
                Log.Debug("Sent: Added item {pickupableType} to container {itemContainerScenePath}", pickupable.GetTechType(), containerTransform.gameObject.GetHierarchyPath());
            }
        }
        ...
        public void BroadcastItemRemoval(Pickupable pickupable, Transform containerTransform)
        {
            ...
            {
                Log.Debug("Sent: Removed item {pickupableType} from container {itemContainerScenePath}", pickupable.GetTechType(), containerTransform.gameObject.GetHierarchyPath());
            }
        }
        ...
        public void AddItem(GameObject item, NitroxId containerId)
        {
            ...
            if (!owner.HasValue)
            {
                Log.Error("Unable to find inventory container with id {itemContainerId}", containerId);
                return;
            }
            Optional<ItemsContainer> opContainer = InventoryContainerHelper.GetBasedOnOwnersType(owner.Value);
            if (!opContainer.HasValue)
            {
                Log.Error("Could not find container field on GameObject {itemContainerScenePath}", owner.Value.GetHierarchyPath());
                return;
            }
        ...
        public void RemoveItem(NitroxId ownerId, NitroxId itemId)
        {
            ...
            if (!opContainer.HasValue)
            {
                Log.Error("Could not find item container behaviour on object {itemContainerScenePath} with id {itemContainerId}", owner.GetHierarchyPath(), ownerId);
                return;
            }
