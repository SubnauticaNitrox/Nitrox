    ...
    public class Fires
    {
        ...
        public void Create(CyclopsFireData fireData)
        {
            ...
            if (transform2.childCount > 0)
            {
                ...
                {
                    DisplayStatusCode(StatusCode.INVALID_FUNCTION_CALL, $"[Fires.Create Fire already exists at node index {fireData.NodeIndex}! Replacing existing Fire Id {existingFireId} with Id {fireData.CyclopsId}]");
            ...
            {
                DisplayStatusCode(StatusCode.SUBNAUTICA_ERROR, $"[{nameof(CyclopsFireCreatedProcessor)} Cannot create new Cyclops fire! PrefabSpawn component could not be found in fire node! Fire Id: {fireData.FireId} SubRoot Id: {fireData.CyclopsId} Room: {fireData.Room} NodeIndex: {fireData.NodeIndex}]");
            }
