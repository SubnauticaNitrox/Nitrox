...
namespace NitroxClient.GameLogic.Helper
{
    public class InventoryContainerHelper
    {
        ...
        public static bool TryGetOwnerId(Transform ownerTransform, out NitroxId ownerId)
        {
            ...
            if (!parent)
            {
                DisplayStatusCode(StatusCode.SUBNAUTICA_ERROR, "Trying to get the ownerId of a storage that doesn't have a parent");
                ownerId = null;
                return false;
            }
            ...
            else if (LockerRegex.IsMatch(ownerTransform.gameObject.name))
            {
                ...
                if (!locker)
                {
                    DisplayStatusCode(StatusCode.SUBNAUTICA_ERROR, $"Could not find Locker Object: {lockerName}");
                    ownerId = null;
                    return false;
                }
                if (!locker.TryGetComponentInChildren(out StorageContainer storageContainer, true))
                {
                    DisplayStatusCode(StatusCode.SUBNAUTICA_ERROR, $"Could not find {nameof(StorageContainer)} From Object: {lockerName}");
                    ownerId = null;
                    return false;
                }
