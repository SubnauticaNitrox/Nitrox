...
internal sealed partial class uGUI_SceneIntro_IntroSequence_Patch : NitroxPatch, IDynamicPatch
{
    private static LocalPlayer localPlayer;
    private static PlayerCinematics playerCinematics;
    ...
    public uGUI_SceneIntro_IntroSequence_Patch(LocalPlayer lp, PlayerCinematics pc, PlayerManager pm)
    {
        localPlayer = lp;
        playerCinematics = pc;
        playerManager = pm;
    ...
    public static void SkipLocalCinematic(uGUI_SceneIntro uGuiSceneIntro, bool wasNewPlayer)
    {
        ...
        {
            Destroy(radioLiveMixin.loopingDamageEffectObj);
        }
        ...
        if (introFireHolder) // Can be null if called very early
        {
            ...
            DestroyImmediate(introFireHolder.gameObject); // Like in Fire.Extinguished() but without delay
        }
        ...
        playerCinematics.SetLocalIntroCinematicMode(IntroCinematicMode.COMPLETED);
    }
    ...
    private static bool AnyKeyDownOrSubstitute()
    {
        return GameInput.AnyKeyDown ||
               ...
               localPlayer.IntroCinematicMode == IntroCinematicMode.COMPLETED;
    }
    ...
    private static float GetSkipTime()
    {
        ...
        return localPlayer.IntroCinematicMode == IntroCinematicMode.SINGLEPLAYER ? Time.time : -1f;
    }
    ...
    private static bool IsRemoteCinematicReady(uGUI_SceneIntro uGuiSceneIntro)
    {
        ...
        if (localPlayer.IntroCinematicMode == IntroCinematicMode.COMPLETED)
        {
        ...
        if (!packetSend)
        {
            ...
            playerCinematics.SetLocalIntroCinematicMode(IntroCinematicMode.WAITING);
            packetSend = true;
            IsWaitingForPartner = true;
            return false;
        }

        SessionId? opPartnerId = playerCinematics.IntroCinematicPartnerId;

        if (localPlayer.IntroCinematicMode == IntroCinematicMode.START &&
            opPartnerId.HasValue && playerManager.TryFind(opPartnerId.Value, out RemotePlayer newPartner))
        {
            partner = newPartner;
            EnqueueStartCinematic(uGuiSceneIntro);
            playerCinematics.SetLocalIntroCinematicMode(IntroCinematicMode.PLAYING);
            playerCinematics.IntroCinematicPartnerId = null;
        }
    ...
    private static void EndRemoteCinematic()
    {
        if (IsPartnerValid())
        {
            ...
            {
                DestroyImmediate(introCinematicUpdater);
    ...
    private static bool IsPartnerValid() => partner != null && playerManager.Find(partner.SessionId).HasValue;
}
