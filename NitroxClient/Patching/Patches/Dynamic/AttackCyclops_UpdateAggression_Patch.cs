...
/// </summary>
internal sealed partial class AttackCyclops_UpdateAggression_Patch : NitroxPatch, IDynamicPatch
{
    ...
    public AttackCyclops_UpdateAggression_Patch(SimulationOwnership so, IPacketSender ps)
    {
    ...
    public static void Postfix(AttackCyclops __instance)
    {
        if (__instance.currentTarget && __instance.currentTarget.TryGetNitroxId(out NitroxId targetId) &&
            __instance.TryGetNitroxId(out NitroxId creatureId) && simulationOwnership.HasAnyLockType(creatureId))
        {
            float aggressiveToNoise = __instance.aggressiveToNoise.Value;

            packetSender.Send(new AttackCyclopsTargetChanged(creatureId, targetId, aggressiveToNoise));
        }
    }
