...
internal sealed partial class ToggleLights_SetLightsActive_Patch : NitroxPatch, IDynamicPatch
{
    private static FMODWhitelist fmodWhitelist;
    ...
    {
        {
            ...
            {
                OpCode = OpCodes.Call,
                Operand = new(nameof(Utils), nameof(Utils.PlayEnvSound))
            },
            "ReplacePlayEnvSound"
        },
    };
    ...
    {
        {
            ...
            {
                OpCode = OpCodes.Call,
                Operand = new(nameof(Utils), nameof(Utils.PlayFMODAsset))
            },
            "ReplacePlayFMODAsset"
        },
    };

    public ToggleLights_SetLightsActive_Patch(FMODWhitelist fw)
    {
        fmodWhitelist = fw;
    }
    ...
    private static void PlayEnvSound3D(FMOD_StudioEventEmitter eventEmitter, Vector3 position, float soundRadiusObsolete)
    {
        ...
        if (!fmodWhitelist.TryGetSoundData(assetPath, out SoundData eventEmitterSoundData))
        {
    ...
    {
        if (!fmodWhitelist.TryGetSoundData(asset.path, out SoundData fmodAssetSoundData))
        {
