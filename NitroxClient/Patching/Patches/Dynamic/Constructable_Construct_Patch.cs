...
internal sealed partial class Constructable_Construct_Patch : NitroxPatch, IDynamicPatch
{
    private static BuildingService buildingService;
    private static IPacketSender packetSender;
    private static ThrottledPacketSender throttledPacketSender;
    ...
    public Constructable_Construct_Patch(BuildingService bs, IPacketSender ps, ThrottledPacketSender tps, EntityMetadataManager emm)
    {
        buildingService = bs ?? throw new ArgumentNullException(nameof(bs));
        packetSender = ps ?? throw new ArgumentNullException(nameof(ps));
        throttledPacketSender = tps ?? throw new ArgumentNullException(nameof(tps));
    ...
    public static void ConstructionAmountModified(Constructable constructable, bool constructing)
    {
        ...
        float amount = Mathf.Clamp01(constructable.constructedAmount);
        ...
        if (amount == 1f)
        {
            throttledPacketSender.RemovePendingPackets(entityId);
            ...
            packetSender.Send(new ModifyConstructedAmount(entityId, 1f));
            return;
        }
        ...
        {
            throttledPacketSender.RemovePendingPackets(entityId);
            packetSender.Send(modifyConstructedAmount);
            return;
        }

        // update as a normal module
        throttledPacketSender.SendThrottled(modifyConstructedAmount, packet => { return packet.GhostId; }, 0.1f);
    }
    ...
    public static IEnumerator BroadcastObjectBuilt(ConstructableBase constructableBase, NitroxId entityId)
    {
        ...
        if (parentBase)
        {
            ...
            {
                buildingService.FailedOperations++;
            ...
            if (moduleObject)
            {
                ...
                {
                    builtPiece = InteriorPieceEntitySpawner.From(builtModule, entityMetadataManager);
                }
        ...
        else
        {
            ...
            buildingService.EnsureTracker(entityId).ResetToId();

            packetSender.Send(new PlaceBase(entityId, BuildEntitySpawner.From(targetBase, entityMetadataManager)));
        }
    ...
    {
        List<Entity> childEntities = BuildUtils.GetChildEntities(@base, baseId, entityMetadataManager);
        ...
        buildingService.EnsureTracker(baseId).LocalOperations++;
        int operationId = buildingService.GetCurrentOperationIdOrDefault(baseId);
