...
internal sealed partial class BaseDeconstructable_Deconstruct_Patch : NitroxPatch, IDynamicPatch
{
    public static readonly MethodInfo TARGET_METHOD = Reflect.Method((BaseDeconstructable t) => t.Deconstruct());

    private static TemporaryBuildData Temp => BuildingService.Main.Temp;
    ...
    public static void PieceDeconstructed(BaseDeconstructable baseDeconstructable, ConstructableBase constructableBase, Base @base, bool destroyed)
    {
        ...
        {
            BuildingService.Main.EnsureTracker(baseId).FailedOperations++;
        ...
        BuildingService.Main.EnsureTracker(baseId).LocalOperations++;
        int operationId = BuildingService.Main.GetCurrentOperationIdOrDefault(baseId);
