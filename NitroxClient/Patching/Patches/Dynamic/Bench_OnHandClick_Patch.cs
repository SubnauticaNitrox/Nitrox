...
internal sealed partial class Bench_OnHandClick_Patch : NitroxPatch, IDynamicPatch
{
    ...
    public Bench_OnHandClick_Patch(SimulationOwnership so, LocalPlayer lp)
    {
    ...
    private static void ReceivedSimulationLockResponse(NitroxId id, bool lockAcquired, HandInteraction<Bench> context)
    {
        ...
        if (lockAcquired)
        {
            ...
            localPlayer.BroadcastBenchChanged(id, BenchChanged.BenchChangeState.SITTING_DOWN);
            skipPrefix = false;
        }
        else
        {
