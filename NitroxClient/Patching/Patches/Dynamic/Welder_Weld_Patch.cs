...
internal sealed partial class Welder_Weld_Patch : NitroxPatch, IDynamicPatch
{
    ...
    public static bool Prefix() => RESPONSE_WELDER == null;

    public Welder_Weld_Patch(SimulationOwnership so, LocalPlayer lp)
    {
        ...
        if ((live.IsAlive() || live.canResurrect) && live.health < live.maxHealth)
        {
            ...
            if (vehicle && live.TryGetIdOrWarn(out NitroxId id))
            {
                ...
                else
                {
                    ...
                    localPlayer.BroadcastWeld(id, addHealth);
                }
            }
            else
            {
