...
internal sealed partial class uGUI_MainMenu_Start_Patch : NitroxPatch, IPersistentPatch
{
    private static IMultiplayerSession multiplayerSession;
    private static JoinServerBackend joinServerBackend;

    public uGUI_MainMenu_Start_Patch(IMultiplayerSession ms, JoinServerBackend jsb)
    {
        multiplayerSession = ms ?? throw new ArgumentNullException(nameof(ms));
        joinServerBackend = jsb ?? throw new ArgumentNullException(nameof(jsb));
    }
    ...
    public static void Postfix()
    {
        ...
        for (int i = 0; i < args.Length; i++)
        {
            if (args[i].Equals("--instantlaunch", StringComparison.OrdinalIgnoreCase) && args.Length > i + 1)
            {
                ...
                _ = joinServerBackend.StartDetachedMultiplayerClientAsync(IPAddress.Loopback, SubnauticaServerConstants.DEFAULT_PORT, SessionConnectionStateChangedHandler).ContinueWithHandleError(ex => Console.WriteLine($"[Nitrox] [ERROR] {ex}"));
            }
        }
#endif
    }
