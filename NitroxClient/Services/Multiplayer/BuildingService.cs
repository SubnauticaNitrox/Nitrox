...
internal sealed partial class BuildingService(EntityMetadataManager entityMetadataManager, IPacketSender packetSender) : IMultiplayerGameService
{
    ...
    public void Update()
    {
        ...
        {
            working = true;
            StartCoroutineDetached(SafelyTreatNextBuildCommand());
        }
    }
    ...
    public IEnumerator BuildModule(PlaceModule placeModule)
    {
        ...
        {
            entityMetadataManager.ApplyMetadata(result.value.Value.gameObject, moduleEntity.Metadata);
        }
    ...
    public void DeconstructionAllowed(NitroxId baseId, ref bool result, ref string reason)
    {
        if (BasesCooldown.ContainsKey(baseId))
        {
            result = false;
            reason = Language.main.Get("Nitrox_ErrorRecentBuildUpdate");
        }
        else if (EnsureTracker(baseId).IsDesynced() && NitroxPrefs.SafeBuilding.Value)
        {
            result = false;
...
/// <summary>
/// Building resync-related part of <see cref="BuildingService"/>.
/// </summary>
partial class BuildingService
{
    ...
    {
        if (!MonoBehaviours.Multiplayer.Main || !MonoBehaviours.Multiplayer.Main.InitialSyncCompleted)
        {
            return;
        }
        ...
        packetSender.Send(new BuildingResyncRequest());
