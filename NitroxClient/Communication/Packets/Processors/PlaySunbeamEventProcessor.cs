...
public class PlaySunbeamEventProcessor : ClientPacketProcessor<PlaySunbeamEvent>
{
    public override void Process(PlaySunbeamEvent packet)
    {
        ...
        if (beginIndex == -1)
        {
            DisplayStatusCode(StatusCode.SUBNAUTICA_ERROR, $"Couldn't find the corresponding sunbeam event in {nameof(PlaySunbeamEvent.SunbeamGoals)} for key {packet.EventKey}");
            return;
        }
