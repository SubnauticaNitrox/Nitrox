        ...
        public override void Process(PDAEntryProgress packet)
        {
            ...
            using (packetSender.Suppress<PDAEntryProgress>())
            {
                ...
                if (PDAScanner.GetPartialEntryByKey(techType, out entry))
                {
                    ...
                    {
                        Log2.Instance.Log(NLogType.Info, "PDAEntryProgress Upldate Old:" + entry.unlocked + " New" + packet.Unlocked);
                        entry.unlocked = packet.Unlocked;
                    }
                }
                else
                {
                    Log2.Instance.Log(NLogType.Info, "PDAEntryProgress New TechType:" + techType + " Unlocked:" + packet.Unlocked);
