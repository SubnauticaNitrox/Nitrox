...
public static class NitroxEntityExtensions
{
        ...
        [CallerLineNumber] int lineNumber = 0)
    {
        ...
        {
            DisplayStatusCode(StatusCode.INVALID_VARIABLE_VAL, $"[{filePath[(filePath.LastIndexOf("\\", StringComparison.Ordinal) + 1)..^2] + methodName}():L{lineNumber}] Couldn't find an id on {gameObject.GetFullHierarchyPath()}");
            nitroxId = null;
            return false;
        }
        ...
        [CallerLineNumber] int lineNumber = 0)
    {
        ...
        {
            DisplayStatusCode(StatusCode.INVALID_VARIABLE_VAL, $"[{filePath[(filePath.LastIndexOf("\\", StringComparison.Ordinal) + 1)..^2] + methodName}():L{lineNumber}] Couldn't find an id on {component.GetFullHierarchyPath()}");
            nitroxId = null;
            return false;
        }
