...
public class PlayerStatsBroadcaster : MonoBehaviour
{
    ...
    public void Awake()
    {
        ...
        if (!survival)
        {
            DisplayStatusCode(StatusCode.REMOTE_PLAYER_ERR, $"Couldn't find the {nameof(Survival)} instance on the main {nameof(Player)} instance. Destroying {nameof(PlayerStatsBroadcaster)}");
            Destroy(this);
        }
    }
