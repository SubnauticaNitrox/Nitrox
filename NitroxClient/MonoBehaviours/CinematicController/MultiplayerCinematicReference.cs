...
public class MultiplayerCinematicReference : MonoBehaviour
{
    ...
    public void CallStartCinematicMode(string key, int identifier, RemotePlayer player)
    {
        ...
        {
            DisplayStatusCode(StatusCode.INVALID_VARIABLE_VAL, $"There was no entry for the key {key} at {gameObject.GetFullHierarchyPath()}");
        }

        if (!controllers.TryGetValue(identifier, out MultiplayerCinematicController controller))
        {
            DisplayStatusCode(StatusCode.INVALID_VARIABLE_VAL, $"There was no entry for the identifier {identifier} at {gameObject.GetFullHierarchyPath()}");
        }
