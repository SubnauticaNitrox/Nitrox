...
public class RemotePlayerVitals : MonoBehaviour
{
    ...
    public static RemotePlayerVitals CreateForPlayer(RemotePlayer remotePlayer)
    {
        ...
        {
            DisplayStatusCode(StatusCode.MISC_UNHANDLED_EXCEPTION, ex.ToString() + $"Encountered an error while creating vitals for player {remotePlayer.PlayerId}, destroying them.");
    ...
    private void CreateStats(Canvas canvas)
    {
        ...
        if (!uGUI)
        {
            DisplayStatusCode(StatusCode.SUBNAUTICA_ERROR, $"[{nameof(RemotePlayerVitals)}] Couldn't find uGUI main instance when creating vitals");
        }
    ...
    private class Bar : IDisposable
    {
        ...
        private void ThrowIfDisposed()
        {
            if (isDisposed)
            {
                DisplayStatusCode(StatusCode.INVALID_FUNCTION_CALL, "Tried to update visual on a disposed player stat.");
            }
        }
    }
}
