...
internal static class TranspilerHelper
{
    ...
    {
        // Copied from HarmonyLib.CodeInstruction
        string FormatOperand(object argument) =>
            argument switch
            {
                null => "NULL",
                MethodBase member => member.FullDescription(),
                FieldInfo fieldInfo => $"{fieldInfo.FieldType.FullDescription()} {fieldInfo.DeclaringType.FullDescription()}::{fieldInfo.Name}",
                Label argLabel => $"Label{argLabel.GetHashCode()}",
                Label[] argLabels => $"Labels{string.Join(", ", argLabels.Select(l => l.GetHashCode().ToString()))}",
                LocalBuilder argLocalBuilder => $"{argLocalBuilder.LocalIndex} ({argLocalBuilder.LocalType})",
                string argString => argString.ToLiteral(),
                _ => argument.ToString().Trim()
            };
        ...
        foreach (CodeInstruction ci in matcher.Instructions())
        {
            sb.AppendFormat("{0,-10}  ", ci.opcode);
