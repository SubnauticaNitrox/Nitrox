...
using UnityEngine;
using Packets = NitroxModel.Packets;
    ...
    public class ToggleLights_SetLightsActive_Patch : NitroxPatch, IDynamicPatch
    {
        ...
        public static void Postfix(ToggleLights __instance, bool __state)
        {
            if (__state != __instance.lightsActive)
            {
                ...
                {
                    Resolve<Vehicles>().GetVehicle<SeamothModel>(id).LightOn = __instance.lightsActive;
                }
                SendPacket<Packets.ToggleLights>(new(id, __instance.lightsActive));
            }
        }
