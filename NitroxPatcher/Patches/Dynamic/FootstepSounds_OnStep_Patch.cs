...
{
    private static readonly MethodInfo targetMethod = Reflect.Method((FootstepSounds t) => t.OnStep(default));
    ...
    // This method is called very often and should therefor be performant
    private static float CalculateVolume(float originalVolume, FootstepSounds instance, FMODAsset asset, Transform xform)
    {
        ...
        if (asset.path.Equals("event:/sub/exo/step", StringComparison.Ordinal) && (!Player.main.currentMountedVehicle || Player.main.currentMountedVehicle.gameObject != instance.gameObject))
        {
    ...
    {
        Resolve<FMODWhitelist>().TryGetSoundData("event:/sub/exo/step", out SoundData soundData);
