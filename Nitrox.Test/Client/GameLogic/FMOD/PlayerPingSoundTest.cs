...
[TestClass]
public sealed class PlayerPingSoundTest
{
    [TestMethod]
    public void BundledAudioBankMatchesPacketAndUsesMonoPcmWave()
    {
        PlayerPingSound.AudioFileNames.Should().HaveCount(PlayerPingCreated.VOICE_LINE_COUNT);
        PlayerPingSound.AudioFileNames.Should().Equal(
            "player_ping_here.wav",
            "player_ping_what_about_here.wav");

        for (int voiceLineIndex = 0; voiceLineIndex < PlayerPingCreated.VOICE_LINE_COUNT; voiceLineIndex++)
        {
            string audioFilePath = PlayerPingSound.GetAudioFilePath(voiceLineIndex);
            File.Exists(audioFilePath).Should().BeTrue($"ping voice line {voiceLineIndex} should exist at '{audioFilePath}'");
    ...
    private static WaveFormat ReadWaveFormat(BinaryReader reader)
    {
        ...
        Assert.Fail("The bundled ping voice-line WAV has no format chunk");
        return default;
    }
