...
internal sealed class BanListCommand(BanService banService) : ICommandHandler
{
    ...
    {
        IReadOnlyList<BanService.BanEntry> bans = await banService.GetActiveBansAsync();
        if (bans.Count == 0)
        {
        ...
        foreach (BanService.BanEntry ban in bans)
        {
            ...
            builder.AppendLine($"{playerName} ({ban.IP}) - {expiry}{reason}");
        }
