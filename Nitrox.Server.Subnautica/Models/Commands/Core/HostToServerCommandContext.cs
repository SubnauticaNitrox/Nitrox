    ...
    public async ValueTask SendToOthersAsync<T>(T data)
    {
        switch (data)
        {
            case Packet packet:
                ...
                break;
            case string message when !string.IsNullOrWhiteSpace(message):
                await packetSender.SendPacketToOthers(new ChatMessage(ChatMessage.SERVER_ID, message), OriginId);
                break;
            default:
                ICommandContext.ThrowNotSupportedData(data);
                break;
    ...
    public async ValueTask SendAsync<T>(SessionId sessionId, T data)
    {
        switch (data)
        {
            case Packet packet:
                ...
                break;
            case string message when !string.IsNullOrWhiteSpace(message):
                await packetSender.SendPacket(new ChatMessage(ChatMessage.SERVER_ID, message), sessionId);
                break;
            default:
                ICommandContext.ThrowNotSupportedData(data);
                break;
