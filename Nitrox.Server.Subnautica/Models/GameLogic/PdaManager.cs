...
internal sealed class PdaManager(ILogger<PdaManager> logger) : ISummarize
{
    ...
    {
        yield return await Task.FromResult($"Known tech: {PdaState.KnownTechTypes.Count}");
        yield return await Task.FromResult($"Encyclopedia entries: {PdaState.EncyclopediaEntries.Count}");
    }
}
