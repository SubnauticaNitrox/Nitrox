...
internal sealed class TimeService : BackgroundService, ISummarize, IHibernate
{
    ...
    public TimeService(PlayerManager playerManager, NtpSyncer ntpSyncer)
    {
    ...
    {
        yield return await Task.FromResult($"Current time: day {GameDay} ({Math.Floor(GameTime.TotalSeconds)}s)");
    }
