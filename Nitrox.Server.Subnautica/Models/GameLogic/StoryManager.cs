...
internal sealed class StoryManager : ISummarize
{
    ...
    {
        yield return await Task.FromResult($"Aurora's state: {GetAuroraStateSummary()}");
        yield return await Task.FromResult($"Scheduled goals stored: {StoryGoalData.ScheduledGoals.Count}");
        yield return await Task.FromResult($"Story goals completed: {StoryGoalData.CompletedGoals.Count}");
        yield return await Task.FromResult($"Unplayed radio messages: {StoryGoalData.RadioQueue.Count}");
