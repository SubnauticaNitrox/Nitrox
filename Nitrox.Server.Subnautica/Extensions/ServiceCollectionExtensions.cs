...
internal static partial class ServiceCollectionExtensions
{
    ...
    public static ILoggingBuilder AddNitroxLogging(this ILoggingBuilder builder)
    {
        ...
        return builder
               ...
               .AddZLoggerInMemory((options, _) =>
               {
                   options.UseNitroxFormatter(formatOptions =>
                   {
                       formatOptions.ColorBehavior = LoggerColorBehavior.Disabled;
                   });
               }, processor => processor.MessageReceived += async log => await ServersManagementService.LogQueue.Writer.WriteAsync(log))
    ...
    {
        // Hack: Save service strongly depends on WorldService so it's a Func<WorldService> to prevent StackOverflow. TODO: Remove need for WorldService; each service should load its own data.
