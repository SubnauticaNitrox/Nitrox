...
internal class BatchEntitySpawnerService(
    ...
    : IHostedService
{
    ...
    public List<Entity> LoadUnspawnedEntities(NitroxInt3 batchId, bool fullCacheCreation = false)
    {
        ...
        if (entities.Count == 0)
        {
            // TODO: Is it necessary to blacklist empty batches (and prevent serialization) or not?
    ...
    private IEnumerable<Entity> SpawnEntitiesUsingRandomDistribution(EntitySpawnPoint entitySpawnPoint, List<UwePrefab> prefabs, DeterministicGenerator deterministicBatchGenerator, Entity parentEntity = null)
    {
        ...
        if (worldEntityFactory.TryFindAsync(chosenPrefab.ClassId, out UweWorldEntity uweWorldEntity))
        {
    ...
    private List<UwePrefab> FilterAllowedPrefabs(List<UwePrefab> prefabs, EntitySpawnPoint entitySpawnPoint, out float fragmentProbability, out float completeFragmentProbability)
    {
        ...
        for (int i = 0; i < prefabs.Count; i++)
        {
            ...
            if (prefab.ClassId != "None" && worldEntityFactory.TryFindAsync(prefab.ClassId, out UweWorldEntity uweWorldEntity) &&
    ...
    {
        if (worldEntityFactory.TryFindAsync(entitySpawnPoint.ClassId, out UweWorldEntity uweWorldEntity))
        {
    ...
    private IEnumerable<Entity> CreateEntityWithChildren(EntitySpawnPoint entitySpawnPoint, string classId, NitroxTechType techType, bool prefabZUp, int cellLevel, NitroxVector3 localScale, DeterministicGenerator deterministicBatchGenerator,
                                                         Entity parentEntity = null, bool randomPosition = false)
    {
        ...
        {
            if (prefabResources.RandomPossibilitiesByClassId.TryGetValue(classId, out string[] choices))
            {
                classId = choices.GetXorRandom(classId);
            }
        ...
        if (parentEntity == null) // Ensures children are only returned at the top level
        {
            ...
            foreach (Entity childEntity in GetDescendants(spawnedEntity))
            {
    ...
    private IEnumerable<Entity> GetDescendants(Entity entity)
    {
        foreach (Entity child in entity.ChildEntities)
        {
            ...
            {
                foreach (Entity childOfChild in GetDescendants(child))
                {
        ...
        for (int i = 0; i < groupAsset.PrefabAssets.Length; i++)
        {
            ...
            if (prefabAsset is PrefabPlaceholderAsset placeholderAsset && placeholderAsset.EntitySlot.HasValue)
            {
                ...
                if (spawnedEntity != null)
                {
                    ...
                    if (prefabResources.PrefabPlaceholdersGroupPaths.ContainsKey(spawnedEntity.ClassId))
                    {
